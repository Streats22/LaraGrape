...
<header class="site-header bg-gradient-to-b from-primary-50 via-white to-secondary/10 shadow-lg border-b-2 border-primary-200">
            ...
            <nav class="hidden md:flex space-x-8">
                @foreach(\App\Models\Page::inMenu()->get() as $menuPage)
                    <a href="{{ route('page.show', $menuPage->slug) }}" 
                       class="text-primary-700 hover:text-primary-600 px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200">
                        {{ $menuPage->title }}
                    </a>
                @endforeach
            </nav>
    ...
    <div x-show="mobileMenuOpen" 
        ...
        <div class="px-2 pt-2 pb-3 space-y-1">
            @foreach(\App\Models\Page::inMenu()->get() as $menuPage)
                <a href="{{ route('page.show', $menuPage->slug) }}" 
                   class="block px-3 py-2 rounded-md text-base font-medium text-primary-700 hover:text-primary-600 hover:bg-primary-50 transition-colors duration-200">
                    {{ $menuPage->title }}
                </a>
            @endforeach
